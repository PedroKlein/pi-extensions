...
export function buildConsolidationPrompt(
  ...
  if ((currentFacts && currentFacts.length > 0) || (currentLessons && currentLessons.length > 0)) {
    ...
    if (currentFacts && currentFacts.length > 0) {
      // Pass ALL keys so the model knows the full namespace.
      // Include values only for the most relevant subset to save tokens.
      parts.push(`The user already has ${currentFacts.length} facts stored. ALL existing keys (do NOT re-extract these unless the value genuinely changed):`);
      parts.push("```");
      parts.push(currentFacts.map(f => f.key).join("\n"));
      parts.push("```");

      // Top-N facts with values for context (pick those most likely relevant to this session)
      // Simple heuristic: show the first 30 with values truncated
      const topN = currentFacts.slice(0, 30);
      if (topN.length > 0) {
        parts.push("\nSample facts with values (for context on existing coverage):");
        for (const f of topN) {
          parts.push(`- ${f.key}: ${f.value.length > 100 ? f.value.slice(0, 100) + "…" : f.value}`);
        }
      }
    }
    if (currentLessons && currentLessons.length > 0) {
      parts.push(`\nThe user has ${currentLessons.length} lessons stored. Recent lessons (do NOT duplicate):`);
      // Show last 40 lessons (most recent = most relevant for dedup)
      const recentLessons = currentLessons.slice(-40);
      for (const l of recentLessons) {
        parts.push(`- [${l.category}] ${l.rule.length > 120 ? l.rule.slice(0, 120) + "…" : l.rule}`);
      }
      if (currentLessons.length > 40) {
        parts.push(`- ... (${currentLessons.length - 40} more lessons not shown)`);
      }
    }
