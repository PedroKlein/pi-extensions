...
export class MemoryStore {
  ...
  private migrate(): void {
    ...
    // Dream tables: state tracking and session dedup
    this.db.exec(`
  ...
  /**
   * AND-based semantic search. Takes pre-extracted terms (not a raw query string).
   * Uses top-5 longest terms for specificity. Falls back to OR for ≤2 terms.
   * Post-filters by match ratio > 0.3 to remove marginal results.
   */
  searchSemanticAnd(terms: string[], limit: number = 8): SemanticEntry[] {
    if (terms.length === 0) return [];

    // Top 5 most specific terms (longest = most discriminating)
    const topTerms = [...terms].sort((a, b) => b.length - a.length).slice(0, 5);

    if (!this.hasFTS5) return this._searchSemanticAndFallback(topTerms, limit);

    // 1-2 terms: OR (AND too strict with few terms); 3+: AND
    const ftsQuery = topTerms.length <= 2
      ? topTerms.map(t => `"${t.replace(/"/g, '""')}"`).join(" OR ")
      : topTerms.map(t => `"${t.replace(/"/g, '""')}"`).join(" AND ");
    ...
    try {
      const rows = this.db.prepare(`
        ...
        ORDER BY bm25(semantic_fts)
        LIMIT ?
      `).all(ftsQuery, limit * 3) as unknown as SemanticEntry[];

      const lowerTerms = topTerms.map(t => t.toLowerCase());
      return rows
        .filter(entry => {
          const text = `${entry.key} ${entry.value}`.toLowerCase();
          const matched = lowerTerms.filter(t => text.includes(t)).length;
          return matched / lowerTerms.length > 0.3;
        })
        .slice(0, limit);
    } catch {
      return this._searchSemanticAndFallback(topTerms, limit);
    }
  }

  private _searchSemanticAndFallback(topTerms: string[], limit: number): SemanticEntry[] {
    ...
    return all
      .map(entry => {
        const text = `${entry.key} ${entry.value}`.toLowerCase();
        const matched = lowerTerms.filter(t => text.includes(t)).length;
        return { entry, score: matched / lowerTerms.length };
      })
  ...
  /**
   * AND-based lesson search. Takes pre-extracted terms (not a raw query string).
   * Uses top-5 longest terms. Falls back to OR for ≤2 terms.
   * Post-filters by match ratio > 0.3 against rule + category.
   */
  searchLessonsAnd(terms: string[], limit: number = 5): LessonEntry[] {
    if (terms.length === 0) return [];

    // Top 5 most specific terms (longest = most discriminating)
    const topTerms = [...terms].sort((a, b) => b.length - a.length).slice(0, 5);

    if (!this.hasFTS5) return this._searchLessonsAndFallback(topTerms, limit);
    ...
    try {
      ...
      return rows
        .filter(row => {
          const text = `${row.rule} ${row.category}`.toLowerCase();
          const matched = lowerTerms.filter(t => text.includes(t)).length;
          return matched / lowerTerms.length > 0.3;
        })
        .slice(0, limit)
        .map(r => ({ ...r, negative: !!r.negative }));
    } catch {
      return this._searchLessonsAndFallback(topTerms, limit);
    }
  }

  private _searchLessonsAndFallback(topTerms: string[], limit: number): LessonEntry[] {
    if (topTerms.length === 0) return [];
    ...
    return all
      .map(entry => {
        const text = `${entry.rule} ${entry.category}`.toLowerCase();
        const matched = lowerTerms.filter(t => text.includes(t)).length;
        return { entry: { ...entry, negative: !!entry.negative } as LessonEntry, score: matched / lowerTerms.length };
      })
      .filter(({ score }) => score > 0.3)
      .sort((a, b) => b.score - a.score)
      .slice(0, limit)
  ...
  getDreamState(key: string): string | null {
    const row = this.db.prepare("SELECT value FROM dream_state WHERE key = ?").get(key) as { value: string } | undefined;
    return row?.value ?? null;
  }

  setDreamState(key: string, value: string): void {
    this.db.prepare(`
  ...
  acquireDreamLock(pid: number, expiresInMs: number = 3600_000): boolean {
    const expiresAt = new Date(Date.now() + expiresInMs).toISOString();

    // Check if lock is held by a living process
    const existing = this.db.prepare("SELECT pid, expires_at FROM dream_lock WHERE id = 1").get() as { pid: number; expires_at: string } | undefined;
    ...
    if (existing) {
      ...
      if (!expired && !samePid) {
        // Lock held by another process and not expired
        return false;
      }
      // Expired or same PID — reclaim
    }

    this.db.prepare(`
      INSERT INTO dream_lock (id, pid, started_at, expires_at)
  ...
  releaseDreamLock(pid: number): void {
    this.db.prepare("DELETE FROM dream_lock WHERE id = 1 AND pid = ?").run(pid);
  }

  isDreamLocked(): boolean {
    const row = this.db.prepare("SELECT expires_at FROM dream_lock WHERE id = 1").get() as { expires_at: string } | undefined;
    if (!row) return false;
  ...
  isSessionProcessed(path: string): boolean {
    const row = this.db.prepare("SELECT 1 FROM processed_sessions WHERE session_path = ?").get(path);
    return !!row;
  }

  markSessionsProcessed(paths: string[], runId: string): void {
    const stmt = this.db.prepare(
      "INSERT OR IGNORE INTO processed_sessions (session_path, dream_run_id) VALUES (?, ?)"
    );
  ...
  /**
   * Unmark sessions processed by a specific dream run.
   * Used to recover from failed runs that incorrectly marked sessions.
   */
  unmarkSessionsByRunId(runId: string): number {
    const result = this.db.prepare(
      "DELETE FROM processed_sessions WHERE dream_run_id = ?"
    ).run(runId);
    return (result as any).changes ?? 0;
  }

  getUnprocessedSessionCount(allPaths: string[]): number {
    if (allPaths.length === 0) return 0;
    let count = 0;
    const stmt = this.db.prepare("SELECT 1 FROM processed_sessions WHERE session_path = ?");
    for (const path of allPaths) {
      if (!stmt.get(path)) count++;
    }
    return count;
  }
