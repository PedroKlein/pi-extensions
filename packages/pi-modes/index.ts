...
const MODE_ORDER: Mode[] = ["ask", "brainstorm", "plan", "build", "none"];

const MODE_LABELS: Record<Mode, { icon: string; label: string; color: "muted" | "warning" | "accent" | "success" | "dim" }> = {
...
const READONLY_BASH_MODES: Set<Mode> = new Set(["ask", "brainstorm", "plan"]);

/** Modes that block mutating subagents */
const RESTRICTED_SUBAGENT_MODES: Set<Mode> = new Set(["ask", "brainstorm", "plan"]);
const BLOCKED_SUBAGENTS = new Set(["worker", "oracle-executor"]);

/** Modes that enforce write filtering (only allow markdown) */
const WRITE_FILTERED_MODES: Set<Mode> = new Set(["ask", "brainstorm"]);
...
function isWriteAllowed(inputPath: string, cwd: string): boolean {
